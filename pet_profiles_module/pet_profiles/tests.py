...
@override_settings(PET_PROFILES_DEMO_MODE=False)
class PetProfileTests(TestCase):
    ...
    def test_edit_medical_summary(self):
        response = self.client.post(
            reverse(
                "pet_profiles:medical_summary_edit",
                kwargs={"pk": self.pet.pk},
            ),
            {
                "current_conditions": "Seasonal allergies",
                "emergency_notes": "Call owner first",
            },
        )
        self.assertEqual(response.status_code, 302)
        self.pet.medical_summary.refresh_from_db()
        self.assertEqual(
            self.pet.medical_summary.current_conditions,
            "Seasonal allergies",
        )
